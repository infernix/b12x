"""Measure ordinary and coupled-Hadamard QSRT W4A8 decode.
...
the same fixed-rate payload, scales, routes, scratch capacity, native MXFP8
activation format, and CUDA graph replay protocol.
"""
...
def main() -> None:
    ...
    coupled_signs = (
        torch.randint(0, 2, (e, 3 * i), dtype=torch.int32, device=device)
        .mul_(2)
        .sub_(1)
        .to(torch.float16)
    )
