...
def test_binding_owned_cache_forwards_planned_recipe(
    ...
    assert calls["model_type"] == ModelType.GLM_NEXT
    assert calls["scale_format"] == ScaleFormat.NVFP4_E4M3
    assert calls["fp8_rope"] is False
    assert calls["latent_scale_per_token"] is True
...
def test_glm_next_public_cpu_reference_path_preserves_model_identity() -> None:
    ...
    assert callable(sparse_mla.concat_and_cache_nvfp4_mla_fp8_rope)
...
def test_glm_next_nvfp4_prefill_2051_matches_dequantized_record_oracle() -> None:
    ...
    activity_before = tuple(stats_before.get(key, 0) for key in allocator_counters)
    graph.replay()
    torch.cuda.synchronize(device)
    stats_after = torch.cuda.memory_stats(device)
    activity_after = tuple(stats_after.get(key, 0) for key in allocator_counters)
